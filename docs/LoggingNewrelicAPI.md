...
func main() {
    ...
    resp, r, err := apiClient.LoggingNewrelicAPI.CreateLogNewrelic(ctx, serviceID, versionID).Name(name).Placement(placement).FormatVersion(formatVersion).ResponseCondition(responseCondition).Format(format).Token(token).Region(region).Execute()
    if err != nil {
...
func main() {
    ...
    apiClient := fastly.NewAPIClient(cfg)
    ctx := fastly.NewAPIKeyContextFromEnv("FASTLY_API_TOKEN")
    resp, r, err := apiClient.LoggingNewrelicAPI.UpdateLogNewrelic(ctx, serviceID, versionID, loggingNewrelicName).Name(name).Placement(placement).FormatVersion(formatVersion).ResponseCondition(responseCondition).Format(format).Token(token).Region(region).Execute()
    if err != nil {
