...
func main() {
    ...
    resp, r, err := apiClient.LoggingLogshuttleAPI.CreateLogLogshuttle(ctx, serviceID, versionID).Name(name).Placement(placement).FormatVersion(formatVersion).ResponseCondition(responseCondition).Format(format).Token(token).URL(url).Execute()
    if err != nil {
...
func main() {
    ...
    apiClient := fastly.NewAPIClient(cfg)
    ctx := fastly.NewAPIKeyContextFromEnv("FASTLY_API_TOKEN")
    resp, r, err := apiClient.LoggingLogshuttleAPI.UpdateLogLogshuttle(ctx, serviceID, versionID, loggingLogshuttleName).Name(name).Placement(placement).FormatVersion(formatVersion).ResponseCondition(responseCondition).Format(format).Token(token).URL(url).Execute()
    if err != nil {
