...
func main() {
    ...
    resp, r, err := apiClient.LoggingPubsubAPI.CreateLogGcpPubsub(ctx, serviceID, versionID).Name(name).Placement(placement).FormatVersion(formatVersion).ResponseCondition(responseCondition).Format(format).User(user).SecretKey(secretKey).AccountName(accountName).Topic(topic).ProjectID(projectID).Execute()
    if err != nil {
...
func main() {
    ...
    apiClient := fastly.NewAPIClient(cfg)
    ctx := fastly.NewAPIKeyContextFromEnv("FASTLY_API_TOKEN")
    resp, r, err := apiClient.LoggingPubsubAPI.UpdateLogGcpPubsub(ctx, serviceID, versionID, loggingGooglePubsubName).Name(name).Placement(placement).FormatVersion(formatVersion).ResponseCondition(responseCondition).Format(format).User(user).SecretKey(secretKey).AccountName(accountName).Topic(topic).ProjectID(projectID).Execute()
    if err != nil {
