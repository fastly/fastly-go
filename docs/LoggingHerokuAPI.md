...
func main() {
    ...
    resp, r, err := apiClient.LoggingHerokuAPI.CreateLogHeroku(ctx, serviceID, versionID).Name(name).Placement(placement).ResponseCondition(responseCondition).Format(format).LogProcessingRegion(logProcessingRegion).FormatVersion(formatVersion).Token(token).URL(url).Execute()
    if err != nil {
...
func main() {
    ...
    apiClient := fastly.NewAPIClient(cfg)
    ctx := fastly.NewAPIKeyContextFromEnv("FASTLY_API_TOKEN")
    resp, r, err := apiClient.LoggingHerokuAPI.UpdateLogHeroku(ctx, serviceID, versionID, loggingHerokuName).Name(name).Placement(placement).ResponseCondition(responseCondition).Format(format).LogProcessingRegion(logProcessingRegion).FormatVersion(formatVersion).Token(token).URL(url).Execute()
    if err != nil {
