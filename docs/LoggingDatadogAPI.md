...
func main() {
    ...
    resp, r, err := apiClient.LoggingDatadogAPI.CreateLogDatadog(ctx, serviceID, versionID).Name(name).Placement(placement).FormatVersion(formatVersion).ResponseCondition(responseCondition).Format(format).Region(region).Token(token).Execute()
    if err != nil {
...
func main() {
    ...
    apiClient := fastly.NewAPIClient(cfg)
    ctx := fastly.NewAPIKeyContextFromEnv("FASTLY_API_TOKEN")
    resp, r, err := apiClient.LoggingDatadogAPI.UpdateLogDatadog(ctx, serviceID, versionID, loggingDatadogName).Name(name).Placement(placement).FormatVersion(formatVersion).ResponseCondition(responseCondition).Format(format).Region(region).Token(token).Execute()
    if err != nil {
